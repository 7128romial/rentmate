...
def swipe():
    ...
    if direction in ('right', 'up'):
        ...
        # DEMO MODE: Auto-approve matches if the user toggled it in settings
        demo_mode = data.get('demoMode', False)
        if demo_mode and not is_match:
            is_match = True
...
def extract_property():
    ...
    system_msg = {
        'role': 'system',
        'content': (
            "אתה עוזר חכם של RentMate שעוזר למשכיר/ה לתאר את הדירה שהוא/היא מפרסם/ת להשכרה. "
            "המשתמש/ת הוא/היא הבעלים של הדירה — הוא/היא יודע/ת מה יש בדירה ומתאר/ת את העובדות עליה. "
            "המשתמש/ת לא מחפש/ת דירה, לא מביע/ה העדפות ולא מחליט/ה מה היה רוצה שיהיה — אלא מספר/ת מה קיים בפועל. "
            "דבר עברית, קצר וידידותי, וניסוח השאלות תמיד מנקודת מבט של תיאור עובדתי של הדירה. "
            "דוגמאות לניסוח נכון: 'מה הכתובת של הדירה?', 'כמה חדרים יש בדירה?', 'באיזו קומה היא?', "
            "'מה השטח במ\"ר?', 'מתי הדירה פנויה לכניסה?', 'אילו מאפיינים יש בדירה — מרפסת, מזגן, חניה, מעלית?'. "
            "ניסוחים אסורים: 'מה חשוב לך שיהיה בדירה?', 'מה היית רוצה?', 'איזו דירה את/ה מחפש/ת?', "
            "'מה התקציב שלך?' — אל תשתמש בניסוחים כאלה בשום מקרה. "
            ...
            "שאל אך ורק שאלות שקשורות לדירה עצמה ולמאפייניה. "
            "אל תשאל על המשכיר/ה, על השוכר/ת, על העדפות אישיות, על תקציב אישי, על מקום עבודה, על משפחה, "
            "על סיבת ההשכרה, או על כל נושא שאינו תכונה של הדירה הנוכחית. "
            "אם המשתמש/ת מנסה לדבר על נושא אחר, החזר אותו/ה בעדינות לתיאור הדירה. "
            "בכל תגובה — קרא תמיד לכלי fill_property_form: בשדה extracted שים רק שדות שהמשתמש/ת הזכיר/ה במפורש; "
            "ב-next_question שאל שאלה אחת קצרה ועובדתית על השדה החשוב הבא שחסר; "
            "סמן ready=true רק כשכותרת, כתובת ו-price מולאו. "
            "אל תמציא ערכים. המחיר הוא מספר אחד בלבד (לא טווח). אם המשתמש/ת נותן/ת טווח, בקש/י לבחור מחיר אחד."
        )
    }
